...
Builds the two files in reports/ from the analytics tables:
    ...
    executive_summary.pdf   - title, overview blurb, then the Quarterly
                              ...
                              underneath it. The remaining 10 sections
                              follow, flowing naturally (no forced page
                              break), grouped under three headings - MARKET,
                              FARE, CARRIER. MARKET/FARE tables are capped
                              at ai_layer/summarize.py's BEST_WORST_TOP_N
                              (currently 3); CARRIER tables at
                              CARRIER_BEST_WORST_TOP_N (currently 5) - kept
                              separate so carrier tables can show more rows
                              without affecting the market tables. The
                              "Top Markets by Volume" table is the
                              exception, capped at --top instead.
                              Every table (all 11, plus Quarterly Trend) has
                              ...
                              A standalone "Overall Consensus" paragraph, not tied to
                              any one table, wraps up the report at the end.
...
REPORT_OVERVIEW = (
    ...
    "covering passenger demand, average fares, and carrier competition across every "
    "market in the DOT's DB1B survey. It highlights the quarter's overall trend, the "
    "largest markets by volume, the fastest-growing and declining routes, the biggest "
    "fare movements, and how carriers stack up against each other, with analysis of "
    "each table written by Claude."
)
...
def _pdf_sections(base_metrics: dict, top_markets_n: int) -> dict:
    ...
    market_n = summarize.BEST_WORST_TOP_N
    ...
    best_growth, worst_growth = best_worst(qualifying_markets, "passenger_growth_pct", market_n)
    best_fare, worst_fare = best_worst(qualifying_markets, "fare_change_pct", market_n)
    best_carrier_pax, worst_carrier_pax = best_worst(carriers, "total_passengers", carrier_n)
    best_carrier_fare, worst_carrier_fare = best_worst(qualifying_carriers, "avg_fare", carrier_n)
    best_carrier_share, worst_carrier_share = best_worst(qualifying_carriers, "avg_market_share_pct", carrier_n)
...
def build_pdf(narrative: dict, quarterly_trend_df: pd.DataFrame, sections: dict) -> None:
    ...
    # Section-level headers (MARKET, FARE, CARRIER) - smaller than the 18pt
    # report Title, bigger than the 10pt table sub-headings below, so all
    ...
    # Quarterly Trends: subheader, table, then its blurb right underneath -
    # kept together so the blurb can never get separated from its table.
    story.append(
        KeepTogether(
            [
    ...
    # MARKET / FARE / CARRIER flow naturally after Quarterly Trends - no
    # forced page break, each section just starts wherever it fits. Each
    # table gets subheader, table, then its matching blurb right below it,
    # all kept together so a blurb can never get separated from its table
    # across a page break. The domain heading is bundled into the same
    # KeepTogether block as its first section so it can't get orphaned from
    # every section under it either.
    ...
    # Overall consensus - standalone paragraph at the very end, not tied to
    # any one table.
    story.append(Spacer(1, 0.3 * inch))
    story.append(Paragraph("Overall Consensus", table_title_style))
    story.append(Spacer(1, 0.15 * inch))
    story.append(Paragraph(_escape_for_pdf(narrative.get("Overall Consensus", "")), styles["Normal"]))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "The xlsx has no row limit on any sheet; every other MARKET/FARE table is capped at "
