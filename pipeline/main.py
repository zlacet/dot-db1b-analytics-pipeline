...
def run_transform_and_load() -> None:
    ...
    try:
        ...
        transform.write_cleaned_csv(market_summary_df)
        print(f"Computed {len(carrier_summary_df):,} carrier-quarter rows")
        print(f"Computed {len(fare_variance_df):,} fare-variance rows")

        load.load_all(conn, market_summary_df, carrier_summary_df, fare_variance_df)
    finally:
        conn.close()


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the full airline fare variance ETL pipeline.")
    parser.add_argument(
        "--skip-reports",
        action="store_true",
