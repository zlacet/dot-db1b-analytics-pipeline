-- =====================================================================
-- Airline Fare Variance Platform — Database Schema
...
-- columns (markets are built from origin/dest airport codes, and carrier
...
CREATE TABLE IF NOT EXISTS market_summary (
    ...
    fare_per_mile            NUMERIC(10, 4) NOT NULL,
    fare_change_pct          NUMERIC(14, 4),
    CONSTRAINT pk_market_summary PRIMARY KEY (market, year, quarter)
);

CREATE INDEX IF NOT EXISTS idx_market_summary_year_quarter
    ON market_summary (year, quarter);


-- ---------------------------------------------------------------------
-- carrier_summary
-- Purpose: carrier performance analysis.
-- Granularity: market + carrier + quarter.
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS carrier_summary (
    ...
    passengers              NUMERIC(14, 2)  NOT NULL,
    average_fare            NUMERIC(12, 2)  NOT NULL,
    market_share_pct        NUMERIC(9, 4)   NOT NULL,
    CONSTRAINT pk_carrier_summary PRIMARY KEY (market, carrier, year, quarter)
);
...
-- Purpose: track quarter-over-quarter fare movement per market.
-- Granularity: market + quarter (current quarter, compared to previous).
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS fare_variance_summary (
    market                      VARCHAR(20)     NOT NULL,
    year                        SMALLINT        NOT NULL,
    quarter                     SMALLINT        NOT NULL,
    current_average_fare        NUMERIC(12, 2)  NOT NULL,
    previous_average_fare       NUMERIC(12, 2),
    fare_change_pct             NUMERIC(14, 4),
    current_fare_per_mile       NUMERIC(10, 4)  NOT NULL,
    previous_fare_per_mile      NUMERIC(10, 4),
    fare_per_mile_change_pct    NUMERIC(14, 4),
    CONSTRAINT pk_fare_variance_summary PRIMARY KEY (market, year, quarter)
);

CREATE INDEX IF NOT EXISTS idx_fare_variance_year_quarter
    ON fare_variance_summary (year, quarter);
